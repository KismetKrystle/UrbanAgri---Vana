...
interface IUrbanFarmDLP is IDataLiquidityPool {
    ...
    function addSolution(uint256 communityId, string memory aiSolution) external;

    function logUserInteraction(uint256 communityId, string memory action, string memory details) external;

    function submitFeedback(uint256 communityId, string memory feedback) external;

    function getSolutions(uint256 communityId) external view returns (Solution[] memory);

    function getUserInteractions(uint256 communityId) external view returns (UserInteraction[] memory);
